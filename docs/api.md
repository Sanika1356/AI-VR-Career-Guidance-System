...
For Gemini, keep `GEMINI_API_KEY` server-only and set `GEMINI_MODEL`, `GEMINI_BASE_URL`, and `GEMINI_MAX_OUTPUT_TOKENS` through the server environment. The provider uses Google’s supported GenerateContent endpoint `/v1beta/models/{model}:generateContent`, sends the grounded prompt in `contents`, sets `generationConfig.maxOutputTokens`, and sets `store: false` where supported to avoid provider-side request logging. The repository does not enable billing or create a Google account. A Gemini Free Tier project may provide free input/output tokens for eligible models subject to project-specific rate limits; usage limits and model eligibility must be checked in Google AI Studio. If the configured model returns a confirmed 404, the server checks the authenticated `/v1beta/models` metadata and retries once with an accessible model that advertises `generateContent`; if the key is missing, the quota is exhausted, no supported model is accessible, or the provider returns another error, the API safely falls back rather than exposing provider details.
...
Persists a structured report produced by the browser-side CVsense-compatible Puter flow. The browser separately loads Puter.js, confirms or requests the user’s Puter sign-in, uploads the selected PDF to Puter, sends the file attachment plus the role prompt to the low-latency `anthropic/claude-haiku-4-5` model, tolerates the supported Puter response shapes, and extracts the JSON object before calling this endpoint. Pathfinder authentication remains required for persistence; Puter authentication does not replace the Pathfinder session.

The JSON request contains `fileName`, `companyName`, `jobRole`, optional `preferredOutputs`, and `analysis`. The server treats the report as untrusted input, re-validates all fields and bounded lists, forces the stored provider label to `puter`, and stores only the normalized structured report and user-owned metadata. It does not receive or persist the resume bytes. After the report is persisted, the browser makes a non-blocking best-effort deletion request for the temporary Puter file so cleanup latency does not delay navigation to the results page. Malformed reports return a safe validation error instead of being stored.
