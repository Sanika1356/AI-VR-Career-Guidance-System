...
For Gemini, keep `GEMINI_API_KEY` server-only and set `GEMINI_MODEL`, `GEMINI_BASE_URL`, and `GEMINI_MAX_OUTPUT_TOKENS` through the server environment. The repository does not enable billing or create a Google account. A Gemini Free Tier project may provide free input/output tokens for eligible models subject to project-specific rate limits; usage limits and model eligibility must be checked in Google AI Studio. If the key is missing, the quota is exhausted, or the provider returns an error, the API safely falls back rather than exposing provider details.
