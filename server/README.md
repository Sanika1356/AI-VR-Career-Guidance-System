...
Gemini is the hosted provider path when `GEMINI_ENABLED=true` and `GEMINI_API_KEY` is configured. It uses Google’s supported GenerateContent method at `/v1beta/models/{model}:generateContent`, sends the normalized model resource path, the grounded prompt in `contents`, uses `store: false` to avoid provider-side logging where supported, and bounds output through `generationConfig.maxOutputTokens`. Set `GEMINI_MODEL` (default `gemini-2.5-flash`), `GEMINI_BASE_URL`, and `GEMINI_MAX_OUTPUT_TOKENS` server-side; never place the key in a `VITE_*` variable or commit it. Use a Gemini Free Tier project and do not enable billing if the goal is zero cost; Google still applies project-specific model and request limits. The advisor sends grounded profile, assessment, catalog, roadmap, and recent owned conversation context as untrusted data and asks Gemini for a detailed conversational answer. Local Ollama remains an alternative when `OLLAMA_ENABLED=true`, `OLLAMA_BASE_URL`, and `OLLAMA_MODEL` are reachable. If Gemini/Ollama is disabled, unavailable, returns an error, produces empty output, or is temporarily circuit-open, the API returns a detailed deterministic fallback with `mode: deterministic_fallback` rather than a generic provider error. `AI_RETRY_ATTEMPTS` controls bounded retries, capped at two retries by server configuration, `AI_MAX_RESPONSE_CHARS` bounds the stored and returned answer length with a default of 4,000 characters, and `AI_CIRCUIT_FAILURE_THRESHOLD` / `AI_CIRCUIT_COOLDOWN_MS` bound repeated provider failures before a cooldown probe is allowed. Provider credentials must remain server-side in environment variables. For operational diagnosis, the server emits sanitized `advisor_provider_selected`, `advisor_provider_succeeded`, and `advisor_provider_fallback` events. These include only the selected provider, boolean Gemini/Ollama configuration state, duration, a failure category, and an upstream HTTP status when applicable; they never include prompts, response bodies, URLs containing credentials, or provider keys. A `gemini` selection followed by `authentication`, `quota`, `request_schema`, `model_or_endpoint_not_found`, `upstream_http`, `timeout`, `network`, `response_shape`, or `empty_response` identifies a Gemini call that was attempted but failed. HTTP validation failures may include an allowlisted `providerErrorCode` such as `INVALID_ARGUMENT`; the provider’s response message and body are never logged. A `none` selection indicates that `GEMINI_ENABLED` was not parsed as `true`, the key was missing/blank, and Ollama was disabled. If the configured model returns a confirmed `404`, the server performs a bounded, server-side `/v1beta/models` capability check using the existing private key, selects a model whose metadata advertises `generateContent`, and retries once; it never guesses a model name. The selected fallback model and endpoint path are logged without credentials. A successful Gemini request emits `advisor_provider_succeeded` and returns `mode: provider`. For a real server-side smoke check, run `pnpm --dir server gemini:smoke` only in the backend environment that already contains the private `GEMINI_API_KEY`; it prints only `status`, safe provider failure metadata, and response length, never the key, prompt, or generated answer.
