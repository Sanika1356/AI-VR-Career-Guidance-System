...
Local Ollama is opt-in through `OLLAMA_ENABLED=true`, `OLLAMA_BASE_URL`, and `OLLAMA_MODEL`; local development can enable it when an Ollama process and model are available. Production defaults to provider-disabled deterministic fallback mode, and the Render manifest explicitly sets `OLLAMA_ENABLED=false` because a free Render web service does not contain a localhost Ollama process. `AI_RETRY_ATTEMPTS` controls bounded retries, capped at two retries by server configuration, `AI_MAX_RESPONSE_CHARS` bounds the stored and returned answer length with a default of 4,000 characters, and `AI_CIRCUIT_FAILURE_THRESHOLD` / `AI_CIRCUIT_COOLDOWN_MS` bound repeated provider failures before a cooldown probe is allowed. If Ollama is disabled, unavailable, returns an error, produces empty output, or is temporarily circuit-open, the API returns a detailed deterministic fallback with `mode: deterministic_fallback` rather than a generic provider error. No hosted AI service or paid API key is required, and provider credentials—if a future approved provider is added—must remain server-side in environment variables.
