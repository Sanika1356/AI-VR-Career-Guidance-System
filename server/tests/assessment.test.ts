...
test('getAssessmentQuestions creates an assessment and omits internal scoring weights', async () => {
  const client = new FakeClient((sql) => {
    if (sql.includes('FROM assessment_questions')) {
      return queryResult([
        { question_id: 'question_1', question_text: 'Which activity interests you most?', question_type: 'single-choice', question_order: 1, option_id: 'option_a', option_label: 'Building software', option_order: 1 },
        { question_id: 'question_1', question_text: 'Which activity interests you most?', question_type: 'single-choice', question_order: 1, option_id: 'option_b', option_label: 'Analyzing data', option_order: 2 },
      ]);
    }
    return queryResult([]);
  });
...
test('submitAssessment stores answers and returns deterministic career scores', async () => {
  const client = new FakeClient((sql) => {
    if (sql.includes('FROM assessments')) return queryResult([{ id: 'assessment_demo', status: 'in_progress' }]);
    if (sql.includes('SELECT id FROM assessment_questions')) return queryResult([{ id: 'question_1' }, { id: 'question_2' }]);
    if (sql.includes('FROM assessment_options')) {
      return queryResult([
        { question_id: 'question_1', option_id: 'option_ai', scoring: { career_ai_engineer: 3 } },
        { question_id: 'question_2', option_id: 'option_ai_2', scoring: { career_ai_engineer: 2 } },
      ]);
    }
    return queryResult([]);
  });
  ...
  assert.ok(client.queries.some((query) => query.startsWith('INSERT INTO assessment_answers')));
  assert.ok(client.queries.includes('COMMIT'));
});
...
test('submitAssessment rejects incomplete answers without committing a result', async () => {
  ...
  await assert.rejects(
    submitAssessment('user_demo', 'assessment_demo', [{ questionId: 'question_1', optionId: 'option_a' }], poolFor(client)),
    (error: Error & { statusCode?: number; code?: string }) => error.statusCode === 400 && error.code === 'incomplete_assessment',
  );
...
test('getAssessmentResult returns only the authenticated user result', async () => {
  const client = new FakeClient((sql) => sql.includes('FROM assessment_results')
    ? queryResult([{
        id: 'result_demo',
  ...
  const result = await getAssessmentResult('user_demo', 'result_demo', poolFor(client));
...
test('assessment endpoints require bearer authentication', async () => {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${address.port}/api/assessment/questions`);
