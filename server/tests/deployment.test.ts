...
test("production migration policy applies schema migrations but skips seed and catalog data by default", () => {
  ...
  assert.equal(shouldRunMigration("006_audit_events", false), true);
...
test("Render and Neon deployment templates keep credentials as private placeholders", () => {
  const renderManifest = readFileSync(
    new URL("../render.yaml", import.meta.url),
    "utf8",
  );
  const neonTemplate = readFileSync(
    new URL("../deployment/neon.env.example", import.meta.url),
    "utf8",
  );
  ...
  assert.match(
    renderManifest,
    /buildCommand: pnpm install --frozen-lockfile && pnpm --dir server build/,
  );
  assert.doesNotMatch(
    renderManifest,
  ...
  assert.match(
    renderManifest,
    /startCommand: pnpm --dir server db:migrate && pnpm --dir server start/,
  );
  ...
  assert.equal(
    neonTemplate.includes(
      "DATABASE_URL=postgresql://USER:PASSWORD@HOST/DATABASE?sslmode=require",
    ),
    true,
  );
  assert.match(qualityWorkflow, /pnpm install --frozen-lockfile/);
  assert.match(qualityWorkflow, /pnpm --dir client check/);
  assert.match(qualityWorkflow, /pnpm --dir server test/);
  assert.match(qualityWorkflow, /pnpm audit --prod --audit-level=high/);
  assert.match(qualityWorkflow, /gitleaks\/gitleaks-action/);
});
