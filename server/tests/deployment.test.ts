...
test('Render and Neon deployment templates keep credentials as private placeholders', () => {
  ...
  assert.match(renderManifest, /buildCommand: pnpm install --frozen-lockfile && pnpm --dir server build/);
  assert.doesNotMatch(renderManifest, /buildCommand: pnpm install --no-frozen-lockfile/);
