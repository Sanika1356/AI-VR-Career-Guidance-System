...
class FakeClient implements DatabaseClient {
  constructor(private readonly profileSkills: string[] = ["python"]) {}
  ...
  async query<T extends QueryResultRow = QueryResultRow>(
    ...
    if (sql.includes("FROM profiles")) {
      return queryResult([
        { current_skills: this.profileSkills },
      ]) as QueryResult<T>;
    }
...
test("getSkillGap prioritizes a missing prerequisite before its dependent skill", async () => {
  const response = await getSkillGap(
    "user_demo",
    "career_ai_engineer",
    poolFor(new FakeClient([])),
  );
  assert.equal(response.skills[0]?.priority, "medium");
  assert.deepEqual(response.skills[0]?.blockedBy, ["Python"]);
  assert.equal(response.skills[1]?.priority, "high");
  assert.match(
    response.skills[1]?.priorityReason ?? "",
...
test("skill-gap endpoint requires bearer authentication", async () => {
  ...
  try {
    const response = await fetch(
      `http://127.0.0.1:${address.port}/api/careers/career_ai_engineer/skill-gap`,
    );
