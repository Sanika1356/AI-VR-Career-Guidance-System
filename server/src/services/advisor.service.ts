...
function parseObject(value: Record<string, number> | string | null): Record<string, number> {
  if (!value) return {};
...
function groupCareerRows(rows: CareerRow[]): { id: string; name: string; description: string; skills: string[] } | null {
  const first = rows[0];
  ...
  return {
    ...
    skills: rows.filter((row) => row.skill_name).map((row) => row.skill_name as string),
  };
}

function fallbackAnswer(message: string, careerName: string | undefined, missingSkills: string[]): string {
  const focus = careerName ? ` for ${careerName}` : '';
  const skills = missingSkills.length > 0
    ...
    try {
      const response = await fetch(`${env.ollamaBaseUrl.replace(/\/$/, '')}/api/chat`, {
        ...
        body: JSON.stringify({
          ...
          stream: false,
          messages: [
            { role: 'system', content: 'You are a careful, evidence-aware career advisor.' },
      ...
      if (typeof content !== 'string' || content.trim().length === 0) throw new Error('Ollama returned an empty answer');
      return content.trim();
    } finally {
...
export class FallbackAdvisorProvider implements AdvisorProvider {
  constructor(
    private readonly context: { message: string; careerName?: string; missingSkills: string[] },
  ) {}

  async generate(): Promise<string> {
    return fallbackAnswer(this.context.message, this.context.careerName, this.context.missingSkills);
  }
}
...
export async function chatAdvisor(
  ...
  try {
    ...
    let career: { id: string; name: string; description: string; skills: string[] } | null = null;
    ...
    if (input.careerId) {
      ...
      if (!career) throw new AppError(404, 'career_not_found', 'The selected career does not exist.');
      ...
      roadmap = roadmapResult.rows.map((row) => ({ ...row, completed: Boolean(row.completed) }));
    }
    ...
    if (conversationId) {
      ...
      if (!conversationResult.rows[0]) throw new AppError(404, 'conversation_not_found', 'The conversation does not exist.');
    } else {
