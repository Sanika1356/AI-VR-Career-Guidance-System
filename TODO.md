...
- [ ] Add explicit consent and privacy-notice flows before collecting optional profile, conversation, analytics, or device data.
- [ ] Add account-data export and deletion workflows, including deletion of profile, assessment, recommendation, roadmap, advisor, and analytics records according to the retention policy.
...
- [ ] Add explainable recommendation scoring that shows matched interests, current skills, missing skills, confidence, and meaningful trade-offs instead of only a rank.
...
- [ ] Ground advisor answers in the user’s approved profile, assessment result, career catalog, skill graph, and roadmap. Return citations or source references for catalog-derived claims.
- [ ] Add prompt-injection defenses for profile text, imported career descriptions, advisor messages, and external resources. Treat all retrieved content as untrusted data.
